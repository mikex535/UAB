...
<?php
    if(!isset($_COOKIE["user"])) {
        ...
        if($role != "Admin"){
            ...
            if(count($_POST) != 0) {
                if(preg_match('/^[_A-z0-9]*((-|\s)*[_A-z0-9])*$/',$_POST["add_folder"])){
                    $found_dup = true;
                    while($found_dup){
                        $found_dup = false;
                    ...
                    mysqli_query($conn,"insert into test.reports_tree 
                        values (".$new_id.",'".$_POST["add_folder"]."',
                        'folder',".$_COOKIE["selected_folder_id"].")");
                    header("Location: main_admin.php"); 
                    exit();
                } else {
                    echo "<p>folder name not allowed. Redirecting...</p>";
